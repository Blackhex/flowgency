...
def validate_proposal_schema(meta: dict) -> list[str]:
    errors = []
    if (
        not isinstance(meta.get("execution_agent"), str)
        or not meta["execution_agent"].strip()
    ):
    ...
    for index, item in enumerate(questions, 1):
        ...
        if (
            not isinstance(item.get("prompt"), str)
            or not item["prompt"].strip()
        ):
            errors.append(
                f"Question '{question_id}' requires a non-empty prompt"
            )
        question_type = item.get("type")
        if question_type not in SUPPORTED_QUESTION_TYPES:
            errors.append(
                f"Question '{question_id}' has unsupported type "
                f"'{question_type}'"
            )
            continue
        if question_type == "choice":
            ...
            if not isinstance(raw_options, list) or not raw_options:
                errors.append(
                    f"Question '{question_id}' requires at least one option"
                )
            ...
            for label in labels:
                if labels.count(label) > 1:
                    duplicate = (
                        f"Question '{question_id}' has duplicate option "
                        f"'{label}'"
                    )
    ...
    for item in questions:
        ...
        if question_type == "boolean":
            if answer not in {"approved", "declined"}:
                errors.append(
                    f"Question '{question_id}' requires Approve or Decline"
                )
            ...
            if item.get("multi"):
                if not isinstance(answer, list):
                    errors.append(
                        f"Question '{question_id}' requires a list of "
                        "selections"
                    )
                elif any(value not in labels for value in answer):
                    errors.append(
                        f"Question '{question_id}' has an invalid selection"
                    )
                elif _is_required(item) and not answer:
                    errors.append(
                        f"Question '{question_id}' requires a selection"
                    )
...
def should_execute_decision(
    questions: list[dict], answers: dict, decision_note: str = ""
) -> bool:
    boolean_questions = [
        item for item in questions if item.get("type") == "boolean"
    ]
    if any(
        answers.get(item["id"]) == "approved"
    ...
    for item in questions:
        ...
        if item.get("type") == "choice" and bool(answer):
            return True
        if (
            item.get("type") in OPEN_QUESTION_TYPES
