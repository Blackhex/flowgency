...
**Architecture:** Add three private Windows helper functions to `flowgency/dispatch/install.py` that mirror the existing Linux (systemd) and macOS (launchd) helpers, and wire them into the three public dispatchers that currently return "not yet implemented" for Windows. The helpers use the Task Scheduler 2.0 COM API (`win32com.client.Dispatch("Schedule.Service")`) to register/query/delete a single task named `FlowgencyDispatch` that runs `pythonw -m flowgency.dispatch.run` every N minutes under the interactive logon token.
...
- Task name is exactly `FlowgencyDispatch`, registered in the root folder (`\`).
...
- Modify: `flowgency/dispatch/install.py` (add import, constant, helper)
- Test: `tests/test_dispatch_install.py`

**Interfaces:**
- Produces: module constant `WINDOWS_TASK_NAME = "FlowgencyDispatch"`; function `_windows_python_launcher() -> str` returning the path to `pythonw.exe` (preferred) or `sys.executable` (fallback).
...
In `flowgency/dispatch/install.py`, add `from datetime import datetime` to the imports and add the constant near the other module constants (after `LAUNCHD_PLIST = "com.flowgency.dispatch"`):
...
In `flowgency/dispatch/install.py`, add after the `uninstall_timer` function (before the `# ── Linux` section):
...
```bash
git add pyproject.toml flowgency/dispatch/install.py tests/test_dispatch_install.py
...
**Files:**
- Modify: `flowgency/dispatch/install.py` (`install_timer` Windows branch + `_install_windows`)
- Test: `tests/test_dispatch_install.py`
...
def test_install_windows_registers_task():
    ...
    assert action.Arguments == '-m flowgency.dispatch.run --config "C:\\cfg\\config.yaml"'
...
In `flowgency/dispatch/install.py`, add below `_windows_python_launcher` (still in the Windows section):
...
def _install_windows(config_path: str, interval: int) -> str | None:
    ...
    try:
        ...
        action.Arguments = f'-m flowgency.dispatch.run --config "{config_path}"'
...
In `flowgency/dispatch/install.py`, replace the Windows branch of `install_timer`:
...
**Files:**
- Modify: `flowgency/dispatch/install.py` (`get_timer_status` Windows branch + `_status_windows`)
- Test: `tests/test_dispatch_install.py`
...
In `flowgency/dispatch/install.py`, add below `_install_windows`:
...
In `flowgency/dispatch/install.py`, replace the Windows branch of `get_timer_status`:
...
**Files:**
- Modify: `flowgency/dispatch/install.py` (`uninstall_timer` Windows branch + `_uninstall_windows`)
- Test: `tests/test_dispatch_install.py`
...
In `flowgency/dispatch/install.py`, add below `_status_windows`:
...
In `flowgency/dispatch/install.py`, replace the Windows branch of `uninstall_timer`:
...
3. Open **Task Scheduler** → confirm an `FlowgencyDispatch` task exists in the root folder, triggering every 15 minutes, action `pythonw.exe -m flowgency.dispatch.run --config ...`.
