...
**Goal:** Separate each group's source workspace from its Flowgency-owned state root so no Flowgency record directories are created inside project repositories.
...
- `groups.<id>.path` is the exact Flowgency-owned group-state root.
- Group `path` directly contains `observations`, `proposals`, `decisions`, `locks`, and `logs`; never append `shared`.
- Authoritative durable jobs remain under `flowgency.memory_store/.jobs/<group-id>`.
...
- Project workspaces must remain free of Flowgency-generated record directories.
...
- `flowgency/configuration/group_paths.py` — owns `ResolvedGroupPaths` and all derived group-state directories.

### Configuration authority

- `flowgency/configuration/models.py` — schema version 3, mandatory group roots, relative path resolution.
- `flowgency/configuration/paths.py` — filesystem safety, overlap validation, and storage initialization.
- `flowgency/configuration/store.py` — validate before initialization and atomically encode only valid schema version 3 configs.
- `flowgency/configuration/patches.py` — create and update both group paths.
- `flowgency/configuration/effective.py` — mandatory runtime root union.
- `flowgency/configuration/__init__.py` — export the new path interfaces.

### Runtime and jobs

- `flowgency/jobs/models.py` — version 3 job snapshots with explicit `workspace_root` and `group_root`.
- `flowgency/jobs/resolution.py` — resolve both roots and one shared effective runtime policy.
- `flowgency/jobs/execution.py` — execute in the workspace and persist prompts/logs in the group root.
- `flowgency/jobs/store.py` — place operation locks under `locks`.
- `flowgency/jobs/reconciliation.py` — carry the group root for decision and recovery projection.
- `flowgency/jobs/submission.py` — initialize all configured storage before submission.
- `flowgency/dispatch/run.py` — write scheduler output and markers under group `logs`.
- `flowgency/integrations/models.py` — expose `IntegrationRunRequest.workspace_root`.
- `flowgency/integrations/flowgency/copilot.py`, `flowgency/integrations/flowgency/script.py` — consume the renamed execution root and script placeholder.

### Web and CLI consumers

- `flowgency/web/state.py` — expose resolved roots and directories to route code.
- `flowgency/web/routes/agent_detail.py` — use group-root activity and log directories.
- `flowgency/web/routes/admin_groups.py` — edit and display workspace and group paths separately.
- `flowgency/templates/admin_org_edit.html` — separate labeled path fields.
- `flowgency/templates/admin_groups.html` — display both roots and remove the obsolete initialize action.
- `flowgency/app.py` — move pipeline and log routes to resolved group directories and remove `shared` reload assumptions.
- `flowgency/cli.py` — read pipeline records from group roots and report workspace/group roots accurately.
...
- `skills/flowgency-setup/SKILL.md`, `skills/flowgency-setup/references/templates.md` — generate only the green-field schema.
...
- Produces: `FlowgencyConfig.schema_version: Literal[3]`
...
In `flowgency/configuration/models.py`, define and use:

```python
from typing import Any, Literal

CONFIG_SCHEMA_VERSION = 3
_ROOT_KEYS = {"schema_version", "flowgency", "memory", "groups"}
...
```powershell
git add flowgency\configuration\models.py tests\conftest.py tests\test_config.py tests\test_config_store.py tests\test_surface_contracts.py
...
**Files:**
- Create: `flowgency/configuration/group_paths.py`
...
- Produces: `initialize_storage_directories(config: FlowgencyConfig) -> None`
- Produces: `validate_resolved_paths(config: FlowgencyConfig) -> tuple[ValidationIssue, ...]`
...
def test_resolved_group_paths_have_no_shared_segment(tmp_path, raw_config):
    from flowgency.configuration.group_paths import resolve_group_paths
    from flowgency.configuration.models import parse_config
...
Expected: import failure for `flowgency.configuration.group_paths` and old workspace validation semantics.
...
Export both names from `flowgency/configuration/__init__.py`.

- [ ] **Step 4: Rewrite filesystem validation**

In `flowgency/configuration/paths.py`:
...
```powershell
git add flowgency\configuration\group_paths.py flowgency\configuration\paths.py flowgency\configuration\store.py flowgency\configuration\__init__.py flowgency\web\dependencies.py flowgency\jobs\submission.py tests\test_path_validation.py tests\test_config_store.py tests\test_server.py
...
In `flowgency/configuration/effective.py`, update `_resolve_sandbox`:
...
In `flowgency/jobs/resolution.py`, delete the duplicate `_resolve_runtime_policy` implementation and call:
...
```powershell
git add flowgency\configuration\effective.py flowgency\jobs\resolution.py tests\test_effective_policy.py tests\test_job_submission.py
...
- Modify: `flowgency/integrations/flowgency/script.py`
...
def test_operation_lock_is_under_group_locks(tmp_path):
    from flowgency.jobs.store import group_operation_lock_path
...
In `flowgency/jobs/resolution.py`:
...
In `flowgency/jobs/execution.py`, return context with:
...
Rename the integration request contract in `flowgency/integrations/models.py`:
...
Rename local parameters from `group_path` to `group_root` where they refer to Flowgency state. `_group_path_identity` must compare `snapshot.config.groups[group_id].path`.

In `flowgency/jobs/reconciliation.py`, consume `group_root`:

```python
"group_root": group["group_root"],
```

In `flowgency/app.py` lifespan, pass:
...
```powershell
git add flowgency\jobs\models.py flowgency\jobs\resolution.py flowgency\jobs\execution.py flowgency\jobs\store.py flowgency\jobs\reconciliation.py flowgency\integrations\models.py flowgency\integrations\flowgency\copilot.py flowgency\integrations\flowgency\script.py flowgency\app.py tests\test_job_models.py tests\test_job_submission.py tests\test_job_execution.py tests\test_job_reconciliation.py tests\test_job_authority.py tests\test_instances.py tests\test_memory_recovery.py tests\test_memory_publication.py tests\test_integration_contract.py tests\test_integration_script.py
...
In `flowgency/web/routes/agent_detail.py`, change helper signatures:
...
- [ ] **Step 5: Replace operational `shared` use in `flowgency/app.py`**
...
```powershell
git add flowgency\dispatch\run.py flowgency\web\state.py flowgency\web\routes\agent_detail.py flowgency\app.py tests\test_dispatch_run.py tests\test_logs.py tests\test_agent_status.py tests\test_agent_detail.py tests\test_dashboard.py
...
In `flowgency/cli.py`, make `_resolve_group` return explicit roots:
...
In `flowgency/app.py`, change the current `list_markdown_items` helper to accept a
...
```powershell
rg -n 'group\.path\s*/\s*"shared"|\["shared"\]|/\s*"shared"' flowgency
```
...
```powershell
git add flowgency\app.py flowgency\cli.py tests\test_cli.py tests\test_cli_contract.py tests\test_decision_prompts.py tests\test_execute_decision.py tests\test_decision_verify.py tests\test_proposal_questions.py tests\test_dashboard.py
...
In `flowgency/configuration/patches.py`, define:
...
In `flowgency/web/routes/admin_groups.py`, parse:
...
In `flowgency/templates/admin_org_edit.html`, replace the single path field with:
...
<div>
  ...
  <p>Flowgency-owned observations, proposals, decisions, locks, and logs.</p>
</div>
```

Preserve the existing Tailwind classes from the old field.

In `flowgency/templates/admin_groups.html`, display both labeled paths and remove “Not initialized (shared/ missing)” plus the nonexistent initialize action.

In `flowgency/web/setup_flow.py`, make the setup prompt explicitly require schema version 3 and both group paths:

```python
"Configure schema_version: 3. For every group, set workspace_path to the "
"project execution workspace and path to a disjoint Flowgency-owned group root. "
...
```powershell
git add flowgency\configuration\patches.py flowgency\web\routes\admin_groups.py flowgency\templates\admin_org_edit.html flowgency\templates\admin_groups.html flowgency\web\setup_flow.py tests\test_config_patches.py tests\test_group_settings.py tests\test_setup_flow.py tests\test_interactive_setup.py tests\test_admin_org_sandbox.py
...
```powershell
.venv\Scripts\python -m pytest tests\test_flowgency_setup_skill.py tests\test_repository_boundaries.py tests\test_server.py -q
```
...
- `path` is the Flowgency-owned group root.
...
Update `skills/flowgency-setup/SKILL.md` and its templates so generated configs contain both mandatory paths and never mention conversion.
...
In `flowgency/app.py`, remove `"shared"` from the ignored directory-component set. External group roots are naturally outside the reload root; project-local `shared` has no Flowgency meaning in schema version 3.
...
```powershell
.venv\Scripts\python -m pytest tests\test_flowgency_setup_skill.py tests\test_repository_boundaries.py tests\test_server.py tests\test_cli_contract.py tests\test_setup_flow.py -q
```
...
```powershell
git add CLAUDE.md README.md kb skills examples tests\ui flowgency\app.py tests
