# Flowgency Data Root Setup Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make guided setup ask for one well-explained Flowgency data root first, derive the canonical storage tree, and keep individual path overrides behind one optional grouped review.

**Architecture:** Keep schema version 3 and all runtime storage APIs unchanged. Strengthen the first-run launch prompt and canonical `flowgency-setup` skill as the behavioral contract, then align the setup template and user documentation with that contract. Pytest text-contract tests prevent the launcher, skill, and documentation from drifting back to path-by-path setup.

**Tech Stack:** Python 3.14, FastAPI setup handoff, Markdown Agent Skills, pytest

## Global Constraints

- After read-only project inspection, the Flowgency data root is the first user-facing setup question.
...
- Add no `flowgency.data_root` field, browser setup field, migration, cleanup, or runtime storage behavior.
- Keep `groups.<group-id>.workspace_path` at the project workspace and all Flowgency-owned paths outside it; never create or reference `<workspace_path>/shared`.
...
- `flowgency/web/setup_flow.py`: supplies the root-first storage instructions to every interactive setup integration without changing the request model.
...
- `skills/flowgency-setup/SKILL.md`: owns question ordering, path derivation, opt-in overrides, validation, creation timing, and final reporting.
- `tests/test_flowgency_setup_skill.py`: locks the canonical skill and supporting documentation to the approved setup contract.
- `skills/flowgency-setup/references/templates.md`: connects the friendly root choice to the unchanged schema version 3 fields.
...
The `.github/skills/flowgency-setup` path remains a discovery junction to `skills/flowgency-setup`; do not edit or replace it separately.
...
- Modify: `flowgency/web/setup_flow.py:22-44`
...
def test_build_setup_prompt_requires_root_first_storage_flow(tmp_path: Path) -> None:
    ...
    for phrase in (
        "Flowgency data root the first user-facing question",
...
```python
    return (
        "Use the flowgency-setup skill to configure Flowgency for this project. "
        ...
        "After read-only project inspection, make the Flowgency data root the first user-facing question. "
        ...
        "is a writable real directory that can safely create it, and expand user-home syntax. Use C:\\Flowgency and ~/Flowgency as examples. "
        ...
        "execution workspace and path to a disjoint Flowgency-owned group root. "
...
```powershell
git add tests/test_setup_flow.py flowgency/web/setup_flow.py
git commit -m "feat(setup): require Flowgency data root first"
```
...
**Files:**
- Modify: `tests/test_flowgency_setup_skill.py:39-84`
- Modify: `skills/flowgency-setup/SKILL.md:7-54`
- Modify: `skills/flowgency-setup/SKILL.md:135-164`
...
- Produces: a deterministic conversational contract that stores effective paths in the existing `flowgency.agent_library`, `flowgency.compilation_cache`, `flowgency.memory_store`, `groups.<group-id>.path`, and `groups.<group-id>.workspace_path` fields.
...
def test_setup_asks_for_data_root_before_team_questions():
    ...
    root_question = normalized.index(
        "ask for the flowgency data root as the first user-facing question"
    )
...
```powershell
python -m pytest `
  tests/test_flowgency_setup_skill.py::test_setup_asks_for_data_root_before_team_questions `
  tests/test_flowgency_setup_skill.py::test_setup_derives_canonical_paths_from_one_data_root `
  tests/test_flowgency_setup_skill.py::test_setup_keeps_path_overrides_behind_one_grouped_review `
  -q
```
...
After inspection, ask for the Flowgency data root as the first user-facing question. Explain that it is a separate home for Flowgency-owned data: reusable agent blueprints, disposable compiled projections, semantic memory and durable jobs, and per-group records. The project workspace remains the source and execution location, and `config.yaml` remains at the authoritative path supplied by the launcher.

Accept an existing directory or a new absolute path. Expand user-home syntax. A missing root is valid when its nearest existing parent is a writable real directory that can safely create it. Give `C:\Flowgency` and `~/Flowgency` as examples. Derive these paths in memory without creating them:
...
## 2. Plan The Team And Resolve Flowgency
...
```markdown
After the consolidated path summary is approved, create the approved `flowgency.agent_library` through safe directory operations; do not place blueprints under the project workspace. For each approved role, create:
```
...
```powershell
git add tests/test_flowgency_setup_skill.py skills/flowgency-setup/SKILL.md
git commit -m "feat(setup): derive storage from Flowgency data root"
```
...
**Files:**
- Modify: `tests/test_flowgency_setup_skill.py:4-10`
- Modify: `tests/test_flowgency_setup_skill.py:86-105`
- Modify: `skills/flowgency-setup/references/templates.md:35-55`
...
- Produces: one consistent user-facing explanation that maps `C:/Flowgency` to the unchanged schema version 3 fields.
...
```powershell
python -m pytest tests/test_flowgency_setup_skill.py::test_setup_docs_present_one_data_root_default -q
```

Expected: FAIL because the current template, guide, and README describe separate storage paths rather than an Flowgency data root.
...
````markdown
Default setup starts from one user-selected Flowgency data root. For an Flowgency data root at `C:/Flowgency` and a group ID of `example`, derive:
...
Map those derived paths to the unchanged schema version 3 fields and keep the execution workspace separate from Flowgency-owned state:
````

Keep the existing YAML example immediately after this text. It already contains the exact four expected `C:/Flowgency/...` values.
...
After read-only project inspection, the first question asks for one Flowgency data root. This is a separate home for reusable agent blueprints, disposable compiled projections, semantic memory and durable jobs, and per-group records. The project workspace remains source code and execution context, while `config.yaml` remains at its authoritative path.

The root may be an existing directory or a new absolute path whose nearest existing parent is a writable real directory that can safely create it. For a root at `C:/Flowgency` and group ID `example`, setup derives:
...
`-- groups/example/      -> C:/Flowgency/groups/example
```
...
2. Asks for the Flowgency data root as the first question and derives the canonical global paths.
...
6. Resolves exactly one canonical config with only the supported root sections (`flowgency`, `memory`, and `groups`) and requires `flowgency.agent_library`, `flowgency.compilation_cache`, and `flowgency.memory_store`.
...
```markdown
Start Flowgency, choose the project folder and supported AI integration, complete the flowgency-setup conversation, and return to the dashboard automatically. The first question chooses an existing or new Flowgency data root for all Flowgency-owned runtime data. The [Flowgency Setup Skill](kb/setup-skill.md) then owns group naming, blueprint source, instances, routines, runtime policy, workspaces, memory, validation, and the one atomic config write.

On first run, open `/setup` and hand off the project folder and supported integration to `flowgency-setup`. Setup derives `agent-library`, `compiled-agents`, `memory`, and `groups/<group-id>` beneath the approved root. Advanced users can opt into one grouped path review; the default flow asks no individual storage-path questions.
```
...
```powershell
python -m pytest tests/test_flowgency_setup_skill.py::test_setup_docs_present_one_data_root_default -q
python -m pytest tests/test_flowgency_setup_skill.py tests/test_setup_flow.py -q
```
...
```powershell
git add tests/test_flowgency_setup_skill.py skills/flowgency-setup/references/templates.md kb/setup-skill.md README.md
git commit -m "docs(setup): explain Flowgency data root layout"
```
...
Expected: either no output, or only `?? docs/superpowers/plans/2026-07-23-flowgency-data-root-setup.md` when the plan document was intentionally left outside the implementation commits.