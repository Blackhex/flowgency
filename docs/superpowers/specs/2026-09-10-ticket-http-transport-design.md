...
- `validate_config` enforces that `integration_config.allow_local_network`, when present for a Copilot agent, is a strict YAML boolean. The ticket-only incompatibility remains a run-time check: `validate_run` raises `ticket-local-network-required` for ticket-enabled sandboxed Copilot runs when `integration_config.allow_local_network` is not explicitly `true`, directing the user to either enable the broad-scope runtime checkbox or choose a non-ticket unrestricted run.
