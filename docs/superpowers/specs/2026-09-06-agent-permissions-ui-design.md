...
Keep the existing permission configuration schema, policy resolution, validation, and enforcement semantics. The user approved one supporting addition: read-only available-tool metadata supplied by integrations. This is not an integration settings editor or a change to what integrations enforce.
...
- New permission semantics, a schema migration, deny rules, or startup conversion.
...
The configuration remains `runtime.permissions` with `mode` and `rules`; each rule still has optional `path` and `tools`.
...
4. Permission configuration patch: change only the selected instance's permissions using the existing revision-checked store.
