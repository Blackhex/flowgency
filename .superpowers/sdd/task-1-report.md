...
- `FlowgencyConfig.schema_version: Literal[3]`
...
Updated `flowgency/configuration/models.py` to:

- introduce `CONFIG_SCHEMA_VERSION = 3`
- accept `schema_version` as a canonical root key
- require `FlowgencyConfig.schema_version: Literal[3]`
...
Updated `flowgency/configuration/paths.py` to align with the split roots:
...
Updated `flowgency/configuration/patches.py` so group create/save patch operations persist `workspace_path` alongside `path`. Current admin/group patch flows still accept one path input, so they now mirror that value into both fields to keep saved configs valid under schema v3.
...
- `skills/flowgency-setup/SKILL.md` canonical YAML example to include `schema_version: 3` and split `workspace_path` / `path`
- `tests/test_flowgency_setup_skill.py` to match the canonical v3 skill content
...
- `skills/flowgency-setup/SKILL.md`
- `tests/conftest.py`
...
- `tests/test_flowgency_setup_skill.py`
