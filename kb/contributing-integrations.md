...
An integration also declares whether it supplies live ticket tools to a running agent. Ticket transport is currently provided by the `copilot` integration only, through a worker-owned authenticated loopback MCP HTTP endpoint. An integration that does not supply an equivalent per-launch authenticated ticket transport fails closed for ticket operations, so a workflow bound to it produces no ticket transport. Ticket tools are scoped to Flowgency-owned storage reached over a loopback endpoint: they let an agent inspect, claim, and transition tickets without any workspace write, git or GitHub credential, or `shell` grant, and a new integration must not widen runtime authority to provide them. Do not rely on global bypasses such as `allowOutbound` or `sandboxMcpServers=false`; if an integration needs broader local-network consent for restricted ticket runs, that consent must be explicit, per agent, and default false.

Integrations may also publish a read-only permission tool catalog for the Admin UI. A catalog may be partial; `complete: true` is reserved for a demonstrably closed vocabulary that includes any extension-provided tools, and changing that vocabulary or its targeting rules must mint a new catalog identity version. Providers that cannot prove completeness must say so explicitly and keep unbounded grant editing unavailable rather than implying full YAML expressiveness.
