...
An integration also declares whether it supplies live ticket tools to a running agent. Ticket transport is currently provided by the `copilot` integration only; an integration that does not supply it fails closed for ticket operations, so a workflow bound to it produces no ticket transport. Ticket tools are scoped to Flowgency-owned storage reached over a loopback endpoint: they let an agent inspect, claim, and transition tickets without any workspace write, `gitAuth`, `ghAuth`, or `shell` grant, and a new integration must not widen runtime authority to provide them.
