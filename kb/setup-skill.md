...
Every generated config uses `schema_version: 6` and requires `agency.agent_library`, `agency.compilation_cache`, `agency.memory_store`, and `agency.prompt_store`. Each team has both `workspace_path` (the execution workspace and source repository) and `path` (the Agency-owned team root). The team root is automatically available to restricted agents. Agency never loads or creates `<workspace_path>/shared`; durable jobs live in `agency.memory_store/.jobs`, and operation locks live in `<team.path>/locks`.
...
team display name and stable ID, asks for an initial positive agent count, and
...
project, team, priorities, and current coverage gaps. Setup shows coverage,
