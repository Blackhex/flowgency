...
- The Copilot sandbox injects git and `gh` credentials through its `sandbox.auth.git` and `sandbox.auth.gh` keys, and only for an agent whose policy grants `write` on the workspace root itself, since a filesystem policy cannot stop a push: the write lands on the remote. The measured CLI (`1.0.84-3`) injects these tokens only while the sandbox is enabled, so a read-only ticket agent (a confined policy) is genuinely denied them, while an unconfined run leaves the sandbox off and the credentials cannot be scoped by it.
