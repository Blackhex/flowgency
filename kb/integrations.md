...
Every Copilot job launches in its own private `COPILOT_HOME`, never the operator's shared one. The job's `config.json` is rebuilt from an explicit authentication-only allowlist (`lastLoggedInUser`, `loggedInUsers`, `authProvider`); the operator's `installedPlugins` and any personal `model`/`trustedFolders` preferences are never copied in, and the source file is read, never written. A private home is prepared even when the source config is missing or unreadable, so a job never falls back to the shared home; it launches with an empty config and a genuine CLI authentication failure instead. A malformed or non-object source config fails the launch outright before the agent runs. (The source file itself may carry a `// ...` comment header, which Copilot writes and tolerates; the reader strips whole comment lines before parsing.)
