...
Per-job execution through `CopilotIntegration.run()` launches each job in its own private `COPILOT_HOME`, never the operator's shared one. The job's `config.json` is rebuilt from an explicit authentication-only allowlist (`lastLoggedInUser`, `loggedInUsers`, `authProvider`); the operator's `installedPlugins` and any personal `model`/`trustedFolders` preferences are never copied in, and the source file is read, never written. A private home is prepared even when the source config is missing, so a job never falls back to the shared home; it launches with an empty config and a genuine CLI authentication failure instead. A read error, malformed source, or non-object source config fails the launch outright before the agent runs. (The standalone `prompt()` convenience API and user-driven interactive setup are separate execution paths and do not carry this per-job isolation guarantee. The source file itself may carry a `// ...` comment header, which Copilot writes and tolerates; the reader strips whole comment lines before parsing.)

Every launch also carries `--disable-builtin-mcps` and `--no-auto-update`. The first disables Copilot's own built-in MCP servers; it does not stop a plugin-sourced server the operator has installed, which is why the `installedPlugins` filtering above is required as well, not redundant with it. Flowgency verifies both flags against the installed CLI's own `--help` before every launch and refuses to run when either is unsupported, rather than launching without isolation. An optional nonblank `integration_config.model` renders as its own `--model <value>` argument; when it is absent, no model argument is added or inherited from the operator's personal config.
