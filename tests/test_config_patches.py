...
def test_patch_agent_runtime_preserves_integration_extensions(config_store):
    from agency.configuration.patches import (
        AgentRuntimePatch,
        ToolPolicy,
        patch_agent_runtime,
    ...
    snapshot = config_store.load()
    snapshot.raw["groups"]["newsletter"]["agents"][0]["integration_config"] = {
        "model": "gpt-5",
        "extension": {"temperature": 0.3},
    }
    snapshot.path.write_text(
        yaml.safe_dump(snapshot.raw, sort_keys=False),
        encoding="utf-8",
    ...
    assert updated.raw["groups"]["newsletter"]["agents"][0][
        "integration_config"
    ] == {
        "model": "gpt-5",
        "extension": {"temperature": 0.3},
