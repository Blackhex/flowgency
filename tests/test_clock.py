...
def test_real_clock_falls_back_to_datetime_now(monkeypatch):
    ...
    monkeypatch.delenv("FLOWGENCY_FIXED_NOW", raising=False)
...
def test_invalid_fixed_clock_fails_clearly(monkeypatch):
    clock = _clock()
    monkeypatch.setenv("FLOWGENCY_FIXED_NOW", "not-a-date")

    with pytest.raises(ValueError, match="FLOWGENCY_FIXED_NOW must be a valid ISO-8601 datetime"):
        clock.now()
